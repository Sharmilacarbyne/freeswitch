...
SWITCH_STANDARD_APP(callcenter_function)
{
	...
	switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member_session), SWITCH_LOG_INFO, "Queue [%s] is not answered from mod_callcenter\n", queue_name);
	times = switch_channel_get_timetable(member_channel);
	switch_snprintf(start_epoch, sizeof(start_epoch), "%" SWITCH_TIME_T_FMT, times->created / 1000000);
